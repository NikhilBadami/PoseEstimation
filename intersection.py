...
def check_hand_inside_bounding_box(hand, pts):
    ...
    inside = True
    # Check each coordinate of hand and see if it is in bounds of box
    if hand[0] > pts[3, 0] or hand[0] < 0:
        inside = False
    if hand[1] > pts[2, 1] or hand[1] < 0:
        inside = False
    if hand[2] > pts[1, 2] or hand[2] < 0:
        inside = False
    return inside
